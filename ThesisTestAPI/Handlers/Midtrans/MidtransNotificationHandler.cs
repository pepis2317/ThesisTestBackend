...
namespace ThesisTestAPI.Handlers.Midtrans
{
    public class MidtransNotificationHandler : IRequestHandler<MidtransNotification, (ProblemDetails?, string?)>
    {
        ...
        public async Task<(ProblemDetails?, string?)> Handle(MidtransNotification request, CancellationToken cancellationToken)
        {
            ...
            if(request.transaction_status != MidtransStatuses.REFUND)
            {
            ...
            MidtransStatus? status = null;
            if(request.transaction_status == MidtransStatuses.REFUND)
            {
                status = new MidtransStatus
                {
                    transaction_status = MidtransStatuses.REFUND,
                    fraud_status = MidtransStatuses.ACCEPT
                };
            }
            else
            {
                status = await _midtransService.GetStatusAsync(request.order_id);
            }
            ...
            var isRefund = status.transaction_status == MidtransStatuses.REFUND || status.transaction_status == MidtransStatuses.PARTIALREFUND;
            ...
            else if (isRefund)
            {
                transaction.Status = TransactionStatuses.POSTED;
                transaction.PostedAt = DateTime.Now;
                var refund = await _db.RefundRequests.Include(q=>q.Process).Where(q => q.RefundRequestId == transaction.ReferenceId).FirstOrDefaultAsync();
                if (refund != null)
                {
