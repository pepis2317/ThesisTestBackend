...
namespace ThesisTestAPI.Handlers.Steps
{
    public class GetStepsHandler : IRequestHandler<GetStepsRequest, (ProblemDetails?, PaginatedStepsResponse?)>
    {
        ...
        {
            var steps = await _db.Steps.Skip((request.pageNumber - 1) * request.pageSize).Where(q => q.ProcessId == request.ProcessId).OrderBy(q=>q.CreatedAt).Select(q=>new StepResponse
            {
                StepId = q.StepId,
                Title= q.Title,
