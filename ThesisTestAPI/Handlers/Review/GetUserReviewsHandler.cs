...
namespace ThesisTestAPI.Handlers.Review
{
    public class GetUserReviewsHandler: IRequestHandler<GetUserReviewsRequest, (ProblemDetails?, PaginatedReviewsResponse?)>
    {
        ...
        public GetUserReviewsHandler(ThesisDbContext db, BlobStorageService blobStorageService)
        {
            _db = db;
        ...
        public async Task<(ProblemDetails?, PaginatedReviewsResponse?)> Handle(GetUserReviewsRequest request, CancellationToken cancellationToken)
        {
            var reviews = await _db.UserReviews
                ...
                .Where(q => q.UserId == request.UserId).OrderByDescending(q => q.UserReviewNavigation.CreatedAt).ToListAsync();
            ...
            {
                var isLiked = liked.Where(q => q.LikeNavigation.ContentId == review.UserReviewId).Any();
                var likeCount = likes.Where(q => q.ContentId == review.UserReviewId).Select(q => q.Likes).FirstOrDefault();
                var commentCount = commentCounts.Where(q => q.TargetContentId == review.UserReviewNavigation.ContentId).FirstOrDefault();
                var rating = ratings.Where(q => q.RatingNavigation.ContentId == review.UserReviewNavigation.ContentId).FirstOrDefault();
