...
namespace ThesisTestAPI.Handlers.Shipment
{
    public class GetAllShipmentsHandler : IRequestHandler<GetAllShipmentsRequest, (ProblemDetails?, PaginatedShipmentResponse?)>
    {
        ...
        public GetAllShipmentsHandler(ThesisDbContext db)
        {
            _db = db;
        }
        ...
        {
            var processIds = await _db.Processes.Include(q => q.Request).ThenInclude(q => q.RequestNavigation).Select(q => q.ProcessId).ToListAsync();
